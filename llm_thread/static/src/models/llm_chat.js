...
export class LLMChat extends Record {
  ...
  llmChatView = Record.one("LLMChatView", { inverse: "llmChat", isCausal: true });
  isInitThreadHandled = Record.attr({ default: false });
  initActiveId = Record.attr({ default: null });
  activeThread = Record.one("Thread", { inverse: "activeLLMChat" });
  threads = Record.many("Thread", { inverse: "llmChat" });
  orderedThreads = Record.many("Thread", {
    compute() {
  ...
  defaultLLMModel = Record.one("LLMModel", {
    compute() {
      if (!this.llmModels || !Array.isArray(this.llmModels)) {
        return null;
      }
